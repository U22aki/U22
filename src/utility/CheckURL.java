...
public class CheckURL {
	...
	public static boolean checkURL(String siteUrl){
		...
		if(siteUrl.indexOf("amazon") != -1){
			...
			//パターン２
			//regex = "(https:\\/\\/|http:\\/\\/)www\\.amazon\\.co\\.jp\\/gp\\/(.+?)\\/(.+?)\\/ref=[^/]+?\\?(.+?)=[0-9A-Z]+?&pf_rd_s=desktop-[0-9]+?&pf_rd_r=[0-9A-Z]+?&pf_rd_t=[0-9]+?&pf_rd_p=(.+?)&pf_rd_i=desktop";
			regex = "(https:\\/\\/|http:\\/\\/)www\\.amazon\\.co\\.jp\\/gp\\/[^/]+?\\/[^/]+?\\/[0-9A-Z]+?\\/ref=[^/]+?\\?(.+?)=[^/]+?&qid=[0-9]+?&sr=[0-9]+?-[0-9]+?&pi=[^/]+?&keywords=[^/]+?$";
			checkPattern(regex, siteUrl);
